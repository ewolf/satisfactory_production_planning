// Script to show input rates for each production configuration
...
const targetItem = 'Heavy Modular Frame';
const targetRate = 2;
const selectedBelt = 'Mk.1';
const selectedPipe = 'Mk.1';

const beltCapacity = BELT_TIERS[selectedBelt];
const pipeCapacity = PIPE_TIERS[selectedPipe];

console.log(`\n${'='.repeat(80)}`);
console.log(`Production Analysis for ${targetItem} @ ${targetRate}/min`);
console.log(`Belt: ${selectedBelt} (${beltCapacity}/min), Pipe: ${selectedPipe} (${pipeCapacity} m³/min)`);
...
for (const pc of prodconfs) {
  console.log(`\n${pc.building} → ${pc.item}`);
  console.log(`  Machines: ${pc.machinesNeeded.toFixed(2)} (${Math.ceil(pc.machinesNeeded)} rounded up)`);
  console.log(`  Output: ${pc.rate.toFixed(2)}/min`);
  console.log(`  Inputs:`);

  for (const [inputItem, inputAmount] of Object.entries(pc.recipe.inputs)) {
    ...
    const inputRatePerMachine = inputPerMinute;

    const isFluid = FLUID_ITEMS.has(inputItem);
    const capacity = isFluid ? pipeCapacity : beltCapacity;
    const cappedPerMachine = inputRatePerMachine > capacity;

    const cappedTotal = totalInputRate > capacity;

    let capNote = '';
    if (cappedPerMachine) {
      capNote = ` ⚠️ CAPPED (${inputRatePerMachine.toFixed(2)}/min per machine > ${capacity}/min ${isFluid ? 'pipe' : 'belt'} capacity)`;
    } else if (cappedTotal) {
      capNote = ` ⚠️ TOTAL CAPPED (total ${totalInputRate.toFixed(2)}/min > ${capacity}/min ${isFluid ? 'pipe' : 'belt'} capacity)`;
    }

    console.log(`    - ${inputItem}: ${totalInputRate.toFixed(2)}/min total (${inputRatePerMachine.toFixed(2)}/min per machine)${capNote}`);
  }
}
